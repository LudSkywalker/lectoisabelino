...
-- Schema lectoisabelino
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `lectoisabelino` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci ;
USE `lectoisabelino` ;
...
CREATE TABLE IF NOT EXISTS `lectoisabelino`.`libros_lecto` (
  ...
  `estado_libros_estLibId` INT NOT NULL,
  `categoria_libro_lecto_catLecId` INT(11) NOT NULL,
  PRIMARY KEY (`libLecCodigo`, `estado_libros_estLibId`, `categoria_libro_lecto_catLecId`),
  INDEX `fk_libros_lecto_estado_libros1_idx` (`estado_libros_estLibId` ASC) VISIBLE,
  INDEX `fk_libros_lecto_categoria_libro_lecto1_idx` (`categoria_libro_lecto_catLecId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `lectoisabelino`.`contr_prestamos_libros` (
  ...
  CONSTRAINT `fk_contr_prestamos_libros_libros_lecto1`
    FOREIGN KEY (`libros_lecto_libLecCodigo`)
    REFERENCES `lectoisabelino`.`libros_lecto` (`libLecCodigo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `lectoisabelino`.`elementos_lecto` (
  ...
  `categoria_elementos_catEleId` INT(11) NOT NULL,
  PRIMARY KEY (`eleLecCodigo`, `estado_elementos_estEleId`, `categoria_elementos_catEleId`),
  INDEX `fk_elementos_lecto_estado_elementos1_idx` (`estado_elementos_estEleId` ASC) VISIBLE,
  INDEX `fk_elementos_lecto_categoria_elementos1_idx` (`categoria_elementos_catEleId` ASC) VISIBLE,
